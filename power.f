...
41    NR = NOPTS-ISTART+1
      WRITE(IOEDM,22) QU,N,L,M,NR
22    FORMAT(D12.6,3I3,I5)
...
c RWANG 10/29/19 to provide inut params IN and IOEDM
       OPEN(9,FILE='GRAVE_INPUT')
       OPEN(90,FILE='energy.txt')
      OPEN(909,FILE='switch.txt')
1     FORMAT(I3/I3/E12.6/E12.6/I3/I3/I3/I3/I3)
    ...
    8 FORMAT(1H1,20X,16HSTATE CONSIDERED,3I5,30X,24HRATIO OF NUCLEAR CHAACXX0061
     1RGES,2X,F9.6,/,21X,16(1H*),45X,24(1H*))
    9 FORMAT(40X,21(1H*),13X,21(1H*),/,40X,1H*,2HR=,E17.10,1H*,13X,3H*E=
     1,E17.10,1H*,/,40X,21(1H*),13X,21(1H*),/   ,27X,3HPE=,E17.10,10X,3HACXX0064
...
C
      ...
      NF=NFIN
      rewind(IOEDM)
      READ(IOEDM,*) QU,N,L,ME,NR
      MYINDEX = 100*N+10*L+ME
      write(*,*) N
      READ(IOEDM,*) (RR(I),P(I),AP(I),I=1,NR)
...
C
      ...
      WRITE(6,9) R,E,PE,A ,AG
      WRITE(6,203) (XG(I),I=1,NG)
      WRITE(6,204) (XF(I),I=1,NF)
